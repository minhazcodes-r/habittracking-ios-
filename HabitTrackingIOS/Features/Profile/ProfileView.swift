...
struct ProfileView: View {
            ...
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Text("Built by ").font(.caption2).foregroundColor(.mutedForeground) +
                        Text("MinhazCodes").font(.caption2).foregroundColor(.white)
    ...
    private func submitFeedback() {
        ...
        Task {
            try? await supabase.from("feedback").insert(msg).execute()
